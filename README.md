...
* Python:
    ...
    *  [psycopg2](https://github.com/psycopg/psycopg2): 2.9.0 or newer
* PostgreSQL server:
    * [PostgreSQL](https://www.postgresql.org/) >= 10.21

## <a href=#installation>Installation</a>

Package is uploaded on [PyPI](https://pypi.org/project/PyPgORM/).
...
```python
from pypgorm.model import Model
...
config = dict(host='127.0.0.1', port=5432, user='postgres', password='password', database='test')
...
```python
# case 1
from pypgorm.transaction import Transaction as t
from models.user import User
try:
    t.begin()
...
to prevent this problem, each connection should have ability to auto reconnect, by default pypgorm's each connection will ping mysql server after 3600 seconds idle or sleep.
...
pypgorm has take care about this situation, each connetion will auto re-connect immediately if the connection has gone.
...
As the code slice mentioned above, pypgorm assign one mysql connection for each
